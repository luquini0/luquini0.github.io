  ...
  (function(){
    ...
    // the site's own stroke-icon style rather than literal brand marks, redrawn
    // at a fixed small size so they stay easy to make out.
    var ICON_PATHS = [
    ...
    function loadIcons(cb){
      ...
      ICON_PATHS.forEach(function(icon, i){
        var svg = icon.fill
          ? '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="' + color + '" stroke="none">' + icon.d + '</svg>'
          : '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="' + color + '" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' + icon.d + '</svg>';
        var img = new Image();
      ...
      entries.forEach(function(entry){
        if(entry.isIntersecting){
          if(!sized){
            sized = true;
            resize();
            loadIcons(function(){ iconsReady = true; render(); });
          }
